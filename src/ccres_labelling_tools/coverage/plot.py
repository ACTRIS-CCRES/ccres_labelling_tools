...
def plot_data_coverage_v2(location, site, df, months_start, output_dir, params):
    """_summary_
    ...
    for i, ax in enumerate(axes.flatten()):
        ...
        if i not in [6]:  # noqa: FURB171
            ...
            ax.xaxis.set_major_formatter(DateFormatter("%b"))
            ax.xaxis.set_major_locator(MonthLocator(interval=1))
            ax.set_xlabel(f"{df_prod.index.year[0]}", fontsize=params.asize)
    ...
    plt.tight_layout()
    fig.suptitle(
        f"{location.capitalize()} ({site['lat']}°N, {site['lon']}°E, {site['alt']}m)\nCloudnet Data Availability\n{date_start_analysis.strftime('%b')}-{date_end_analysis.strftime('%b %Y')} analysis",  # noqa: E501
        fontsize=params.tsize,
    )
    add_logo(left=0.78, bottom=0.885, width=0.2, height=0.1)
    plt.subplots_adjust(top=0.86)
